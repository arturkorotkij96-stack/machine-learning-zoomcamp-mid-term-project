...
if __name__ == "__main__":
    ...
    model = LogisticRegression(solver='lbfgs', C = 0.01, max_iter=100000)
    model_lg_s, auc_lg_s = train_model(X_full_train_scaled, Y_full_train, X_test_scaled, Y_test, model)
    print(f"model_lg_s auc: {auc_lg_s}")
    ...
    model = RandomForestClassifier(**params_rf, n_jobs=N_JOBS)
    model_rf, auc_rf = train_model(X_full_train, Y_full_train, X_test, Y_test, model)
    print(f"model_rf auc: {auc_rf}")
    ...
    model = xgb.XGBClassifier(**xgb_params, n_jobs=N_JOBS)
    model_xgb, auc_xgb = train_model(X_full_train, Y_full_train, X_test, Y_test, model)
    print(f"model_xgb auc: {auc_xgb}")
